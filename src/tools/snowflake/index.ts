...
async function executeQuery(sql: string, binds: any[] = []): Promise<any[]> {
...
// Tool definitions
export const snowflakeTools: Tool[] = [
  {
...
// Tool handler
export async function handleSnowflakeTool(name: string, args: any): Promise<any> {
  switch (name) {
    case 'snowflake_run_query': {
      const sql = args?.sql as string;
      const binds = (args?.binds as string[]) || [];
    ...
    case 'snowflake_list_databases': {
      ...
      const databases = rows.map((r: any) => ({ name: r.name, owner: r.owner, created: r.created_on }));
      return { content: [{ type: 'text', text: JSON.stringify(databases, null, 2) }] };
    }

    case 'snowflake_list_schemas': {
      const database = (args?.database as string) || config.database;
      const rows = await executeQuery(`SHOW SCHEMAS IN DATABASE ${database}`);
      const schemas = rows.map((r: any) => ({ name: r.name, database, owner: r.owner }));
    ...
    case 'snowflake_list_tables': {
      ...
      const tables = rows.map((r: any) => ({ name: r.name, schema, database, rows: r.rows, created: r.created_on }));
