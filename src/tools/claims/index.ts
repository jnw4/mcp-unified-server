...
async function executeClaimsQuery(sql: string, binds: any[] = []): Promise<any[]> {
  const connection = connectionManager.getConnection('snowflake');
  if (!connection) {
...
// Tool handler
export async function handleClaimsTool(name: string, args: any): Promise<any> {
  try {
    switch (name) {
      case 'claims_list': {
      ...
      case 'claims_add_citation': {
        ...
        // Add citation
        await executeClaimsQuery(
          `INSERT INTO PUREPLAY.CLAIMS.CITATIONS (id, claim_id, summary, quotation, source_type, source_id, source_link)
           VALUES (?, ?, ?, ?, ?, ?, ?)`,
          [citationId, claim_id, summary, quotation || null, source_type, source_id, source_link || null]
        );

        // Update claim updated_at
        await executeClaimsQuery(
          'UPDATE PUREPLAY.CLAIMS.CLAIMS SET updated_at = CURRENT_TIMESTAMP() WHERE id = ?',
          [claim_id]
        );
      ...
      case 'claims_create_candidate': {
        ...
        const embeddingRows = await executeClaimsQuery(
        ...
        await executeClaimsQuery(
          `INSERT INTO PUREPLAY.CLAIMS.CLAIMS (id, statement, type, context_layer, client_id, status, embedding)
           VALUES (?, ?, ?, ?, ?, 'candidate', ?)`,
          [claimId, statement, type, context_layer, client_id || null, embedding]
        );
        ...
        await executeClaimsQuery(
          `INSERT INTO PUREPLAY.CLAIMS.CITATIONS (id, claim_id, summary, quotation, source_type, source_id, source_link)
           VALUES (?, ?, ?, ?, ?, ?, ?)`,
          [citationId, claimId, summary, quotation || null, source_type, source_id, source_link || null]
        );
      ...
      case 'claims_auto_promote': {
        ...
        for (const candidate of candidates) {
          await executeClaimsQuery(
            'UPDATE PUREPLAY.CLAIMS.CLAIMS SET status = ? WHERE id = ?',
            ['curated', candidate.ID]
          );
          promoted++;
        }
      ...
      case 'claims_classify_context': {
        ...
        const clients = await executeClaimsQuery(
          'SELECT id FROM PUREPLAY.CLAIMS.CLIENTS WHERE domain = ?',
          [domain]
        );
      ...
      case 'claims_get_or_create_client': {
        ...
        // Check existing
        const existing = await executeClaimsQuery(
          'SELECT id, name FROM PUREPLAY.CLAIMS.CLIENTS WHERE domain = ?',
          [domain]
        );
        ...
        await executeClaimsQuery(
          'INSERT INTO PUREPLAY.CLAIMS.CLIENTS (id, name, domain) VALUES (?, ?, ?)',
          [clientId, name || domain, domain]
        );
