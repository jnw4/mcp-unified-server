...
exports.qboTools = void 0;
exports.handleQboTool = handleQboTool;
...
// API helper
async function qboApiCall(endpoint, method = 'GET', body) {
...
// Tool handler
async function handleQboTool(name, args) {
    switch (name) {
        ...
        case 'qbo_set_tokens': {
            const client = getOAuthClient();
            const authResponse = await client.createToken(`${process.env.QBO_REDIRECT_URI}?code=${args.auth_code}&realmId=${args.realm_id}`);
        ...
        case 'qbo_get_pnl': {
            ...
            const params = [];
            if (args.start_date)
                params.push(`start_date=${args.start_date}`);
            if (args.end_date)
                params.push(`end_date=${args.end_date}`);
            if (params.length > 0)
        ...
        case 'qbo_get_purchases': {
            ...
            if (args.vendor) {
                ...
                purchases = purchases.filter((p) => p.EntityRef?.name?.toLowerCase().includes(vendorLower));
            }
            ...
            const summary = purchases.map((p) => ({
                ...
                Lines: p.Line?.filter((l) => l.DetailType === 'AccountBasedExpenseLineDetail').map((l) => ({
                    LineNum: l.LineNum,
                    Amount: l.Amount,
        ...
        case 'qbo_update_purchase_category': {
            ...
            for (const update of args.line_updates) {
                ...
                if (update.line_num) {
                    ...
                    const line = purchase.Line?.find((l) => l.LineNum === update.line_num);
                    if (!line) {
            ...
            const updatedLines = updated.Line?.filter((l) => l.DetailType === 'AccountBasedExpenseLineDetail').map((l) => ({
                LineNum: l.LineNum,
                Amount: l.Amount,
                Description: l.Description || '',
                Category: l.AccountBasedExpenseLineDetail?.AccountRef?.name || 'Uncategorized',
        ...
        case 'qbo_create_invoice': {
            const invoice = {
                ...
                Line: args.line_items.map((item, idx) => ({
                    LineNum: idx + 1,
